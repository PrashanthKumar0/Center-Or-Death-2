...
function init_game() {
    ...
    enemy = new Enemy(
        new Vec2(W / 2, InitialEnemyRadius + SPIKE_AMP),
        InitialEnemyRadius,
        G_DUE_TO_GRAVITY
    );
...
function constraint_enemy(sound = true) {
    ...
    if (enemy.position.x < enemy.radius + SPIKE_AMP) {
        ...
        particleSystem.spawnAt(
            new Vec2(enemy.position.x - enemy.radius, enemy.position.y)
        );
    }
...
function add_listeners() {
    onkeydown = function (e) {
        if (!KEYBOARD.key_presses.includes(e.key.toLocaleLowerCase()))
            KEYBOARD.key_presses.push(e.key.toLocaleLowerCase());
    };
    onkeyup = function (e) {
