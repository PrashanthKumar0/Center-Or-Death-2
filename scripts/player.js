...
class Player {
    ...
    draw(ctx, particle_system, enemy) {
        let player_center = this.position
            .copy()
            .add(new Vec2(this.width / 2, this.height / 2));
        ...
        if (enemy_cannon_dist <= (this.boundingRadius + enemy.radius) ** 2) {
            //approximation
            // TODO: PLAY SOUND TANK BLAST
            let norm = enemy_cannon_diff.normalize();
            enemy.velocity = norm
                .copy()
                .scale(-norm.dot(enemy.velocity))
                .copy();
            enemy.position.add(
                norm
                    .copy()
        ...
        this.bullets.forEach((bullet, idx) => {
            if (bullet.position.x < 0 || bullet.position.x > ctx.canvas.width) {
                ...
                particle_system.spawnAt(
                    bullet.position.copy(),
                    bullet.velocity.copy(),
                    this.maxBulletParticles,
                    this.bulletParticleMaxLife
                );
            ...
            if (
                bullet.position.y < 0 ||
                bullet.position.y > ctx.canvas.height
            ) {
        ...
        if (this.angry)
            ctx.fillStyle =
                "hsla(" +
                Math.floor(Math.abs(360 * Math.sin(this.time))) +
                ",82%,52%,1)";
        ...
        ctx.save();
        {
            // Save
            ctx.translate(
                _cannonX + this.cannonWidth / 2,
                _cannonY + cannonHeight - wheelPad
            );
            ctx.rotate(this.cannonAngle);
            ctx.fillRect(
                -(this.cannonWidth / 2),
                -cannonHeight - wheelPad,
                this.cannonWidth,
                cannonHeight
            ); // cannon barrel

            ctx.fillRect(
                -(this.cannonWidth / 2) - barrelW / 2,
                -cannonHeight - wheelPad - barrelW / 2,
                this.cannonWidth + barrelW,
                this.cannonWidth + barrelW
            ); // barel opening
            ctx.beginPath();
        ...
        this.drawWheel(
            ctx,
            x + wheelR + wheelPad,
            y + this.height - wheelR,
            wheelR,
            dTheta
        ); // Left
        this.drawWheel(
            ctx,
            x + this.width - wheelR - wheelPad,
            y + this.height - wheelR,
            wheelR,
            dTheta
        ); // Right
    ...
    constraintAngle() {
        if (this.cannonAngle > 1.3962634015954636) {
            // 85 degree
    ...
    shootBullet() {
        ...
        pos.add(
            vel
                .copy()
                .normalize()
                .scale(this.cannonHeight + this.cannonWidth)
        );
        ...
        for (let i = 0; i < 8; i++) {
            ctx.fillRect(
                pos.x + _d * (Math.random() - 0.5),
                pos.y + _d * (Math.random() - 0.5),
                this.cannonWidth / 2,
                this.cannonWidth / 2
            );
        }
        ...
        this.bullets.push(
            new Bullet(pos, vel, this.bulletRadius, this.bulletMass)
        );
    }
    ...
    impactEnemy(bullet, bullet_idx, enemy, particle_system) {
        ...
        if (
            collision_normal.magSq() <
            Math.pow(Math.ceil(bullet.radius + enemy.radius), 2)
        ) {
            enemy.applyForce(bullet.velocity.copy().scale(bullet.mass)); // todo: modify it
            enemy.impactMass(bullet.mass);

            collision_normal.normalize();
            let collision_tangent = new Vec2(
                -collision_normal.y,
                collision_normal.x
            );

            collision_normal.scale(collision_normal.dot(bullet.velocity));
            collision_tangent.scale(
                collision_tangent.dot(bullet.velocity) * 0.7
            ); // 0.7 is friction
