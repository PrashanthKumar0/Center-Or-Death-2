...
const _CONTROLS_TANK_FIRE_DELAY = 0.01;
const _CONTROLS_TANK_MOVE_DELAY = 0.3;
const _CONTROLS_TANK_BARREL_DELAY = 0.3;
    ...
    KEYBOARD.key_presses.forEach((key) => {
        switch (key) {
            case "q":
                ...
                SOUND_POOL.play("tank_barrel", _CONTROLS_TANK_BARREL_VOLUME, _CONTROLS_TANK_BARREL_DELAY);
                break;
            case "e":
                player.cannonAngle += 0.1;
                SOUND_POOL.play("tank_barrel", _CONTROLS_TANK_BARREL_VOLUME, _CONTROLS_TANK_BARREL_DELAY); // volume will be same?
                break;
            case "arrowleft":
            case "a":
                player.moveLeft();
                SOUND_POOL.play("tank_move", _CONTROLS_TANK_MOVE_VOLUME, _CONTROLS_TANK_MOVE_DELAY);
                break;
            case "arrowright":
            case "d":
                player.moveRight();
            ...
            case "w":
                player.shootBullet();
                SOUND_POOL.play("tank_fire", _CONTROLS_TANK_FIRE_VOLUME, _CONTROLS_TANK_FIRE_DELAY);
                break;
        }
    });

    if (KEYBOARD.key_presses.length == 0) { // if window looses focus
        SOUND_POOL.pause("tank_move",_CONTROLS_TANK_MOVE_DELAY);
        SOUND_POOL.pause("tank_barrel",_CONTROLS_TANK_BARREL_DELAY);
    }
