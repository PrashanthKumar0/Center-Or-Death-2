...
function handle_keyboard_input() {
    ...
    if (!(KEYBOARD.key_presses.includes('q') || KEYBOARD.key_presses.includes('e'))) {
        SOUND_POOL.pause("tank_barrel");
    }

    if (!(KEYBOARD.key_presses.includes('arrowup') || KEYBOARD.key_presses.includes('arrowleft') || KEYBOARD.key_presses.includes('a') || KEYBOARD.key_presses.includes('arrowright') || KEYBOARD.key_presses.includes('d'))) {
        SOUND_POOL.pause("tank_move");
    }
}
...
function init_touch() {
    ...
    let bR = 45;
    _CONTROLS_SHOOT_BUTTON = new Button(new Vec2(W - bR - 10, H - bR - 10), bR,"🔫");
