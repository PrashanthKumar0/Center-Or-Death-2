...
function setup_audio() {
    ...
    let auds = [
        ...
        new Sound("enemy_destroy", _baseUrl + "Audio/SFX/heavy-beam-weapon-7052.mp3"),
        new Sound("wall_collide", _baseUrl + "Audio/SFX/kick-tech-5825.mp3"),
        new Sound("tank_destroyed", _baseUrl + "Audio/SFX/transmission-activated.wav"),
        new Sound("tank_angry", _baseUrl + "Audio/SFX/plasma-gun-power.wav"),

        new Sound("bgm_main", _baseUrl + "Audio/Music/ES_Fair N Square - William Benckert.mp3"),
    ];


    SOUND_POOL = new SoundPool(auds, on_audio_loaded_callback, on_audio_progress_callback);
}

function on_audio_progress_callback(numLoaded, totalSounds) {
    $('loadStatus').innerHTML += ("Loaded Audio :" + numLoaded + " / " + totalSounds + "<br>");
}
